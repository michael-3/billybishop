...
def reask(departure_date, destination, origin):
    ...
    if (not departure_date) and (not destination) and (not origin):
        ...
        response['message'] = 'Where do you want to go from {0}, and when are you leaving?'.format(origin.title())
    elif (not departure_date) and destination and (not origin):
        response['message'] = 'Nice, {0} is great this time of year! When are you leaving, and where from?'.format(destination.title())
        ...
        response['message'] = 'That\'s a great time to visit {0}! Where will you be departing from?'.format(destination.title())
...
# for testing (for example: ./flight_finder.py "I'm going from Rome to Paris on 2016-12-12" | python -m json.tool)
